...
class RandomBlur:
    ...
    def __call__(self, results, label=False):
        if np.random.uniform() > self.params.get('prob', 1):
            return results

        for key in self.keys:
            results[key] = self._apply_random_blur(results[key], self.params.get('seq_prob', 1))
...
class RandomResize:
    ...
    def _random_resize(self, imgs):
        ...
        fix_scale = self.params.get('fix_scale',False)
        ...
        if target_size is None:
            ...
            if resize_mode == 'up':
                scale_factor = resize_scale[1] if fix_scale else np.random.uniform(1, resize_scale[1])
            elif resize_mode == 'down':
                scale_factor = resize_scale[0] if fix_scale else np.random.uniform(resize_scale[0], 1)
            else:
                scale_factor = 1
...
class RandomNoise:
    ...
    def _apply_random_noise(self, imgs, seq_prob):
        ...
        if noise_type.lower() == 'gaussian':
            imgs = self._apply_gaussian_noise(imgs,seq_prob)
        elif noise_type.lower() == 'poisson':
            imgs = self._apply_poisson_noise(imgs, seq_prob)
        else:
    ...
    def __call__(self, results, label=False):
        ...
        for key in self.keys:
            results[key] = self._apply_random_noise(results[key], self.params.get('seq_prob', 1))
...
class RandomJPEGCompression:
    ...
    def __call__(self, results, label=False):
        ...
        for key in self.keys:
            results[key] = self._apply_random_compression(results[key], self.params.get('seq_prob', 1))
...
class RandomVideoCompression:
    ...
    def _apply_random_compression(self, imgs):
        codec_prob = []
        for i in self.params['codec_prob']:
            if isinstance(i,str):
                i = int(i.split('/')[0]) / int(i.split('/')[1])
            codec_prob.append(i)
...
class DegradationsWithShuffle:
    ...
    def __call__(self, results, skiphalf=False):
        ...
        for degradation in self.degradations:
            if isinstance(degradation, (tuple, list)):
                for subdegrdation in degradation:
                    if skiphalf and isinstance(subdegrdation,RandomVideoCompression):
                        continue
                    if subdegrdation.__repr__().endswith('skiphalf'):
                        results = subdegrdation(results, skiphalf)
                    else:
                        results = subdegrdation(results)
            else:
                if skiphalf and isinstance(degradation,RandomVideoCompression):
                    continue
                if degradation.__repr__().endswith('skiphalf'):
                    results = degradation(results, skiphalf)
                else:
...
@PIPELINE_REGISTRY.register()
class DegradationsWithSampling:
...
@PIPELINE_REGISTRY.register()
class DoubleDegradationsWithSampling:
    """Apply random degradations to input, with degradations being sampled.
    ...
    def __call__(self, results):
        ...
        idx_list1 = [] if n_sample==0 else idx_list[:n_sample]

        # apply degradations to input
        for i,degradation in enumerate(self.degradations):
            if i in idx_list1:
                degradation.keys = [self.keys[0]]
            else:
                degradation.keys = [self.keys[1]]
            results = degradation(results)
