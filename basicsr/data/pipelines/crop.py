...
class FixedCrop:
    ...
    def __init__(self, keys, crop_size, crop_pos=None, scale_factor=4):
        if not mmengine.utils.misc.is_list_of(crop_size, int):
            raise TypeError(
                'Elements of crop_size must be int and crop_size must be'
                f' list, but got {type(crop_size[0])} in {type(crop_size)}')
        if not mmengine.utils.misc.is_list_of(crop_pos, int) and (crop_pos is not None):
            raise TypeError(
                'Elements of crop_pos must be int and crop_pos must be'
                f' list or None, but got {type(crop_pos[0])} in '
                f'{type(crop_pos)}')
    ...
    def __call__(self, results ):
        ...
        if isinstance(results[self.keys[0]], list): # keys[0] == gt
        ...
        for k in self.keys:
            ...
            if k == 'label':
                r = self.scale // 2
                x_offset, y_offset, crop_w, crop_h = (x_offset+r)//self.scale, (y_offset+r)//self.scale, (crop_w+r)//self.scale, (crop_h + r)//self.scale
                for image in images:
...
class CropAroundUnknown:
    ...
    def __init__(self,
        ...
        elif mmengine.utils.misc.is_list_of(interpolations,
