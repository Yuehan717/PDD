...
class SRModel(BaseModel):
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('embed_opt'):
            self.cri_embed = build_loss(train_opt['embed_opt']).to(self.device)
        else:
            self.cri_embed = None
    ...
    def feed_data(self, data):
        ...
        if 'lq_real' in data:
            self.lq_real = data['lq_real'].to(self.device)
        if 'gt' in data:
            self.gt = data['gt'].to(self.device)  # Use gt or gt_unsharp
        if 'gt_unsharp' in data:
            self.gt_unsharp = data['gt'].to(self.device)
        if 'gt_real' in data:
            self.gt_real = data['gt_real'].to(self.device)
        if 'gt_aux' in data:
            self.gt_aux = data['gt_aux'].to(self.device)
        if 'labels' in data:
            self.dg_labels = data['labels'].to(self.device)
    ...
    def optimize_parameters(self, current_iter):
        ...
        # pixel loss
        if self.cri_embed:
            l_embed = self.cri_embed(self.output[1], self.output[2], self.output[3])
            if l_embed is not None:
                l_total += l_embed
                loss_dict['l_embed'] = l_embed
                
        if self.cri_pix:
            if self.cri_pix_name.startswith('Multi'):
                if hasattr(self, 'gt_aux'):
                    loss = self.cri_pix(self.output, self.gt_unsharp, self.gt_aux) if self.is_use_sharpened_gt_in_pixel else self.cri_pix(self.output, self.gt, self.gt_aux)
                else:
                    loss = self.cri_pix(self.output, self.gt_unsharp) if self.is_use_sharpened_gt_in_pixel else self.cri_pix(self.output, self.gt)
                for key in list(loss.keys()):
                    if loss[key] is not None:
                        l_total += loss[key]
                        loss_dict[key] = loss[key]
            else:
                l_pix = self.cri_pix(self.output, self.gt_unsharp) if self.is_use_sharpened_gt_in_pixel else self.cri_pix(self.output, self.gt)
                l_total += l_pix
                loss_dict['l_pix'] = l_pix
        # Grad loss
        if self.cri_grad:
            l_grad = self.cri_grad(self.output, self.gt_unsharp) if self.is_use_sharpened_gt_in_pixel else self.cri_grad(self.output, self.gt)
            l_total += l_grad
            loss_dict['l_grad'] = l_grad
        # perceptual loss
        if self.cri_perceptual:
            l_percep, l_style = self.cri_perceptual(self.output, self.gt_unsharp) if self.is_use_sharpened_gt_in_percep else self.cri_perceptual(self.output, self.gt)
        ...
        if self.cri_class:
            l_class = self.cri_class(self.output, self.dg_labels)
            l_total += l_class
